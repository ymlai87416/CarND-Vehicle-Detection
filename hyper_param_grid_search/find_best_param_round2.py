...
def read_image(imgs, color_space):
    result = []
    for file in imgs:
        file_features = []
...
def evaluate_linear_svc_param(filename, color_space, orient, pix_per_cell, cell_per_block, hog_channel, 
    ...
    with open(filename, 'a') as f:
        f.write("%s, %d, %d, %d, %s, %d, %d, %s, %s, %s, %d, %.4f\n" % (color_space, orient, pix_per_cell, cell_per_block, str(hog_channel), spatial_size[0], hist_bins, spatial_feat, hist_feat, hog_feat, len(X_train[0]), acc_score))
    f.closed
...
if __name__ == '__main__':
    ...
    file = open("./job/"+file_input, "r") 
    
    cars = read_image(cars, color_spacex)
    notcars = read_image(notcars, color_spacex)
    
    for line in file: 
        b = line.split(",")
        color_space = b[0]
        orient = int(b[1])
        pix_per_cell = int(b[2])
        cell_per_block = int(b[3])
        hog_channel = b[4]
        spatial_size = int(b[5])
        hist_bins = int(b[6])
        spatial_feat = b[7]
        hist_feat = b[8]
        hog_feat = b[9]
        ...
        if(color_space == color_spacex):
            t=time.time()
            evaluate_linear_svc_param(file_input+'_r2.txt', color_space, orient, pix_per_cell, cell_per_block, hog_channel, 
                                                spatial_size, hist_bins, spatial_feat, hist_feat, hog_feat) 
            t2 = time.time()
            print(round(t2-t, 2), 'Seconds to extract features and train SVC...')
